    ...
    def __call__(self, xs, positions, attn_mask, kv_cache):
        ...
        for i, (x, config) in enumerate(zip(xs, self.configs, strict=True)):
            ...
            if config.num_kv_heads == config.num_heads:
                ...
                # B; batch, T: token length,  D: token dim (2048), 
                # N: number of heads (8) , H: head dimension/D_h (256)
...
class Block(nn.Module):
    ...
    def __call__(self, xs, kv_cache, positions, attn_mask, adarms_cond, deterministic=True):  # noqa: FBT002
        ...
        post_attn, kv_cache, probs = attn(pre_attn, positions, attn_mask, kv_cache)
...
class Module(nn.Module):
    ...
    ) -> tuple[Sequence[at.Float[at.Array, "b _t _d"] | None], tuple[KVCache, at.Float[at.Array, "layers ..."]]]:
        embedded = jax.tree.map(lambda e: e.astype(self.embed_dtype), embedded)
        mask = jnp.asarray(mask)[:, None, :, :]
        if adarms_cond is None:
            adarms_cond = [None] * len(self.configs)

        embedded, (kv_cache, stacked_probs) = self.layers(embedded, kv_cache, positions, mask, adarms_cond, deterministic)

        assert all(e.dtype == jnp.dtype(self.embed_dtype) for e in embedded if e is not None)

        return [
            f(e, a)[0] if e is not None else e for f, e, a in zip(self.final_norms, embedded, adarms_cond, strict=True)
        ], (kv_cache, stacked_probs)
